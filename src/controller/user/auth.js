...
exports.loginUser = async (req,res) =>{
    try {
        const data = req.body
        const loginData = await loginModel.findOne({name:data.name},{password:1,name:1,_id:1})
        if(!loginData){
        ...
        if(data.password===loginData.password){
         const token = jwt.sign({id:loginData._id,role:Role.User,name:loginData.name},process.env.JWT_Key,{expiresIn:"1h"})
       return  res.json({
            status:"success",
...
exports.ManagerLoginUser = async (req,res) =>{
    try {
        ...
        const tokenCreate = jwt.sign({id:userFind._id,role:Role.User},process.env.JWT_Key,{expiresIn:"1h"})
        if(tokenCreate){
            res.json({
